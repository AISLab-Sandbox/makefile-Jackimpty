...
| config - {operation[0], output_ch[1:0], ofmap_col[4:0], input_ch[1:0]}                          |
|===============================================================================================|
...
always_ff @( posedge clk or posedge rst ) begin // FSM_seq
    if (rst) cs <= IDLE;
    else cs <= ns;
end
...
always_comb begin // FSM_comb
    case (cs)
        ...
        COMPUTE: begin
            if (count_filt_col == FILT_COL && count_input_ch == input_ch && count_ofmap_ch == ofmap_ch) ns = ADD_IPSUM;
            else ns = COMPUTE;
        end
        ADD_IPSUM: begin
...
always_ff @( posedge clk or posedge rst ) begin // SETUP
    if (rst) begin
        input_ch <= 'd0;
        ofmap_col <= 'd0;
        ofmap_ch <= 'd0;
...
// ==================== PE counter ====================
always_ff @( posedge clk or posedge rst ) begin // count_filt_col
...
always_ff @( posedge clk or posedge rst ) begin // count_ofmap_ch
    if (rst) count_input_ch <= 2'd0;
    else begin
        case (cs)
            LOAD_FILT: begin
...
always_ff @( posedge clk or posedge rst ) begin // count_input_ch
...
always_ff @( posedge clk or posedge rst ) begin // count_ofmap_col
...
// ==================== PE spad ====================
always_ff @( posedge clk or posedge rst ) begin // filter_spad
    integer i;
...
always_ff @( posedge clk or posedge rst ) begin // ifmap_spad
    integer i;
    if (rst) for (i = 0; i < 12; i = i + 1) ifmap_spad[i] <= 8'd0;
    else begin
        if (cs == LOAD_IFMAP && ifmap_valid) begin
            ifmap_spad[index_0[3:0]] <= {!ifmap[7], ifmap[6:0]};
            ifmap_spad[index_1[3:0]] <= {!ifmap[15], ifmap[14:8]};
            ifmap_spad[index_2[3:0]] <= {!ifmap[23], ifmap[22:16]};
            ifmap_spad[index_3[3:0]] <= {!ifmap[31], ifmap[30:24]};
        end
...
always_ff @( posedge clk or posedge rst ) begin // ofmap_spad
    if (rst) begin
        for (i = 0; i < `OFMAP_SPAD_LEN; i++) ofmap_spad[i] <= 'd0;
    end
    else begin
        if (cs == COMPUTE) ofmap_spad[ofmap_index] <= add_res;
        else if (cs == ADD_IPSUM && ipsum_valid) begin
            ofmap_spad[ofmap_index] <= add_res;
        end
        else if (cs == LOAD_IFMAP_REUSE) begin
            for (i = 0; i < `OFMAP_SPAD_LEN; i++) ofmap_spad[i] <= 'd0;
        end
    end
end
